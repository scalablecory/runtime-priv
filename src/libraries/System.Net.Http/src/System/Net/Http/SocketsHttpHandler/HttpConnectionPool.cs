...
namespace System.Net.Http
{
    ...
    internal sealed class HttpConnectionPool : IDisposable
    {
        ...
        {
            // TODO: this may not be the best way to check if H3 connection is open yet.
            ...
            GetHttp3ConnectionAsync(HttpRequestMessage request, HttpAuthority authority, CancellationToken cancellationToken)
        {
            ...
            Http3Connection http3Connection = _http3Connection;
            ...
            if (http3Connection != null)
            {
                ...
                {
                    // Connection expired.
                    await http3Connection.DisposeAsync();
            ...
            try
            {
                ...
                QuicConnection quicConnection = await ConnectHelper.ConnectQuicAsync(authority.IdnHost, authority.Port, _sslOptionsHttp3, cancellationToken);

                if (quicConnection.NegotiatedApplicationProtocol != SslApplicationProtocol.Http3)
                {
                    HandleBadAltSvc(authority);
        ...
        public async Task<HttpResponseMessage> SendWithRetryAsync(HttpRequestMessage request, bool doRequestAuth, CancellationToken cancellationToken)
        {
            while (true)
            {
                ...
                if (_altSvcEnabled && response.Headers.AltSvc.Count != 0)
                {
                    HandleAltSvc(response.Headers.AltSvc, response.Headers.Age);
                }
                ...
                {
                    response.Dispose();
                    HandleBadAltSvc(h3Connection.Authority);
                    continue;
                }
        ...
        /// </summary>
        internal void HandleAltSvc(IEnumerable<AltSvcHeaderValue> altSvcHeaderValues, TimeSpan? responseAge)
        {
            HttpAuthority nextAuthority = null;
            TimeSpan nextAuthorityMaxAge = default;

            foreach (AltSvcHeaderValue value in altSvcHeaderValues)
            {
        ...
        /// </summary>
        private void HandleBadAltSvc(HttpAuthority badAuthority)
        {
            Debug.Assert(badAuthority != null);
            Debug.Assert(badAuthority != _originAuthority);

            HashSet<HttpAuthority> collection = _altSvcBlacklist;

            if (collection == null)
            {
                lock (SyncObj)
                {
                    collection = _altSvcBlacklist;
                    if (collection == null)
                    {
                        collection = new HashSet<HttpAuthority>();
                        _altSvcBlacklistTimerCancellation = new CancellationTokenSource();
                        _altSvcBlacklist = collection;
                    }
                }
            }
            ...
            lock (collection)
            {
                added = collection.Add(badAuthority);

                if (added && collection.Count >= MaxAltSvcIgnoreListSize && _altSvcEnabled)
                {
            ...
            if (added)
            {
                _ = Task.Delay(AltSvcBlacklistTimeoutInMilliseconds)
                    .ContinueWith(t =>
                    {
                        lock (collection)
                        {
                            collection.Remove(badAuthority);
                        }
