...
namespace System.Net.Http.Tests
{
    public class AltSvcHeaderParserTest
    {
        ...
        public void TryParse_Success(string value, AlternateService[] expectedServices)
        {
            ...
            while (parseIdx < value.Length)
            {
                ...
                AlternateService actual = Assert.IsType<AlternateService>(result);

                Assert.True(expectedIdx != expectedServices.Length, "More services than expected.");
                AlternateService expected = expectedServices[expectedIdx++];
        ...
        public static IEnumerable<object[]> SuccessfulParseData()
        {
            ...
            yield return new object[]
            {
                "h2=\":8000\"", new []
                {
                    new AlternateService("h2", null, 8000, defaultAge)
                }
            };
            ...
            yield return new object[]
            {
                ...
                {
                    new AlternateService("h2", "new.example.org", 80, defaultAge)
                }
            };
            ...
            yield return new object[]
            {
                ...
                {
                    new AlternateService("h2", "alt.example.com", 8000, defaultAge),
                    new AlternateService("h2", null, 443, defaultAge)
                }
            };
            ...
            yield return new object[]
            {
                "h2=\":443\"; ma=3600", new []
                {
                    new AlternateService("h2", null, 443, TimeSpan.FromTicks(TimeSpan.TicksPerSecond * 3600))
                }
            };

            // Example from RFC 7838, Section 3.1: change of port with max age and persist.
            // (Persist is ignored by our implementation, and should be skipped.)
            yield return new object[]
            {
                "h2=\":443\"; ma=2592000; persist=1", new []
                {
                    new AlternateService("h2", null, 443, TimeSpan.FromTicks(TimeSpan.TicksPerSecond * 2592000))
                }
            };
